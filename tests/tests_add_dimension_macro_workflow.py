...
async def main() -> None:
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if add_dimension.get("reason") != "macro_method_unavailable_on_host":
                raise AssertionError(f"Expected host-stable macro fallback result: {json.dumps(add_dimension, ensure_ascii=False)}")
